...
/*
  std::cout  << "index " << motor_number_
	  << " rot_vel " << motor_rot_vel_
	  << " Force " << link_->GetWorldForce().GetLength()
	  << " Acc " << link_->GetWorldLinearAccel().GetLength() 
	  << " rForce " << link_->GetRelativeForce().GetLength()
	  << " rAcc " << link_->GetRelativeLinearAccel().GetLength() 
	  << " force0 " << force
	  << " force1 " << math::Vector3(0, 0, force * scalar).GetLength()
	  << " constant " << link_->GetRelativeForce().GetLength()/(motor_rot_vel_*motor_rot_vel_)
	  << std::endl;
*/
